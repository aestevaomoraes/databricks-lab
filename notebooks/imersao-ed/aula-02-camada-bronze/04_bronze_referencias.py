...
REF = "/Volumes/voo_bem/bronze/arquivos/referencias"
...
).saveAsTable("voo_bem.bronze.aerodromos")

print(f"bronze.aerodromos: {spark.table('voo_bem.bronze.aerodromos').count():,} linhas")
display(spark.sql("SELECT icao, nome, municipio, uf FROM voo_bem.bronze.aerodromos WHERE icao IN ('SBRB','SBGR','SBSP','SBFZ')"))
...
display(dbutils.fs.ls(REF))

# COMMAND ----------

spark.read.text(f"{REF}/AerodromosPublicos.csv").count()
...
# Passo 1 - Renomear o arquivo atual
dbutils.fs.mv(
    f"{REF}/AerodromosPublicos.csv",
    f"{REF}/AerodromosPublicos_old.csv"
)
...
# MAGIC %md
# MAGIC 2. Validar os nomes das colunas antes de gravar
...
empresas_nacionais = (
    ...
    .load(f"{REF}/pda_empresas_aereas_nacionais.csv")
)

print(empresas_nacionais.columns)
...
empresas_estrangeiras = (
    ...
    .option("skipRows", 1)
    .option("encoding", "UTF-8")
    .load(f"{REF}/pda_empresas_aereas_estrangeiros.csv")
)

print(empresas_estrangeiras.columns)
...
for arquivo, tabela in [
    ("pda_empresas_aereas_nacionais.csv",    "voo_bem.bronze.empresas_nacionais"),
    ("pda_empresas_aereas_estrangeiros.csv", "voo_bem.bronze.empresas_estrangeiras"),
]:
...
# MAGIC Próximo passo
# MAGIC
# MAGIC Antes de avançar para a próxima seção do notebook, eu faria uma validação rápida:

# COMMAND ----------

display(spark.table("voo_bem.bronze.empresas_nacionais"))

# COMMAND ----------

display(spark.table("voo_bem.bronze.empresas_estrangeiras"))
...
codigos.write.format("delta").mode("overwrite").option(
    "overwriteSchema", "true"
).saveAsTable("voo_bem.bronze.codigos_operacao")

print(f"bronze.codigos_operacao: {spark.table('voo_bem.bronze.codigos_operacao').count()} linhas")
display(spark.table("voo_bem.bronze.codigos_operacao"))
...
display(spark.sql("SHOW TABLES IN voo_bem.bronze"))
